...
class WindowService {
    ...
    /// Get all windows including hidden apps and apps without visible windows
    func getAllWindows() -> [WindowInfo] {
        var all = getWindowList()
        all.append(contentsOf: getHiddenApps())
        all.append(contentsOf: getMinimizedWindows())
        all.append(contentsOf: getAppsWithoutVisibleWindows(existingPIDs: Set(all.map { $0.ownerPID })))
        return all
    }
    
    /// Get running apps that don't have any visible windows on screen
    /// These could be apps on other Spaces, or apps with all windows minimized/hidden
    private func getAppsWithoutVisibleWindows(existingPIDs: Set<Int>) -> [WindowInfo] {
        var result: [WindowInfo] = []
        let runningApps = NSWorkspace.shared.runningApplications
        
        for app in runningApps {
            // Only consider regular apps (not background agents, accessories, etc.)
            guard app.activationPolicy == .regular,
                  let appName = app.localizedName else {
                continue
            }
            ...
            // Skip if we already have windows for this app
            ...
            // Skip hidden apps (handled separately in getHiddenApps)
            if app.isHidden {
                continue
            }
            
            // This app is running but has no visible windows - likely on another Space
            // Create a pseudo-entry for it
            let windowInfo = WindowInfo(
                windowNumber: pid * -2,  // Use different negative range to distinguish
                ownerPID: pid,
                ownerName: appName,
                windowName: nil,  // We don't know the window title
                bounds: .zero,
                layer: 0,
                isOnScreen: false,
                isHidden: false,
                isMinimized: false  // We don't know if it's minimized
            )
    ...
    func minimizeWindow(_ window: WindowInfo) {
        ...
        if !isAccessibilityTrusted() {
            print("[WindowService] Accessibility permission not granted. Cannot minimize window.")
            // Request permission (this will show a dialog)
        ...
        guard result == .success, let axWindows = windowsRef as? [AXUIElement], !axWindows.isEmpty else {
            print("[WindowService] Could not get windows for PID \(window.ownerPID), result: \(result.rawValue)")
            return
        }
        ...
        if minimizedSetResult == .success {
            print("[WindowService] Successfully minimized window via AXMinimized attribute")
            return
        }
        
        print("[WindowService] AXMinimized failed with result: \(minimizedSetResult.rawValue), trying minimize button...")
        ...
        if buttonResult == .success, let minimizeButton = minimizeButtonRef as! AXUIElement? {
            let actionResult = AXUIElementPerformAction(minimizeButton, kAXPressAction as CFString)
            if actionResult == .success {
                print("[WindowService] Successfully minimized window via minimize button")
            } else {
                print("[WindowService] Minimize button action failed with result: \(actionResult.rawValue)")
            }
        } else {
            print("[WindowService] Could not get minimize button, result: \(buttonResult.rawValue)")
        }
    }
