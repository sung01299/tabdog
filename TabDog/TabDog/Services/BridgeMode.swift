...
class BridgeMode {
    ...
    func run() {
        fputs("[TabDog Bridge] Starting bridge mode...\n", stderr)
        ...
        fputs("[TabDog Bridge] Bridge mode ended for \(currentBrowser)\n", stderr)
    }
    ...
    private func readLoop() {
        while isRunning {
            do {
                ...
                fputs("[TabDog Bridge] Error reading message: \(error)\n", stderr)
                break
            }
        }
    }
    ...
    private func handleMessage(_ message: IncomingMessage) {
        fputs("[TabDog Bridge] Received: \(message.type)\n", stderr)
        ...
        case "TABS_UPDATE":
            if let data = message.data {
                ...
                if currentBrowser == "unknown" && browser != "unknown" {
                    ...
                    fputs("[TabDog Bridge] Detected browser: \(browser)\n", stderr)
                }
                ...
                fputs("[TabDog Bridge] Wrote \(data.tabs.count) tabs for \(currentBrowser)\n", stderr)
            }
        ...
        case "CONNECTION_STATUS":
            ...
            sharedData.setConnected(true, browser: currentBrowser)
            fputs("[TabDog Bridge] Connection status updated for \(currentBrowser)\n", stderr)
            
        case "ERROR":
            fputs("[TabDog Bridge] Extension error: \(message.message ?? "unknown")\n", stderr)
            
        default:
            fputs("[TabDog Bridge] Unknown message type: \(message.type)\n", stderr)
        }
    }
    ...
    private func sendCommand(_ command: TabCommand) {
        ...
        do {
            ...
            fputs("[TabDog Bridge] Sent command: \(command.type)\n", stderr)
        } catch {
            fputs("[TabDog Bridge] Failed to send command: \(error)\n", stderr)
        }
    }
}
