...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func updateStatusItem() {
        ...
        // Icon: connected -> filled, disconnected -> outline
        let iconName = viewModel.isConnected ? "dog.fill" : "dog"
        button.image = NSImage(systemSymbolName: iconName, accessibilityDescription: "TabDoggy")
    ...
    private func registerGlobalHotkey() {
        ...
        guard status == noErr else {
            print("[TabDoggy] Failed to register global hotkey, status=\(status)")
            return
        }
        ...
        if handlerStatus != noErr {
            print("[TabDoggy] Failed to install hotkey handler, status=\(handlerStatus)")
        }
    }
